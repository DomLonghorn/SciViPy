...
def crystal_script(Filepath):
    ...
    def FrameCreation(NoOfFrames=len(onlyfiles), File, FilePathForScreenshot, ScalarName, opacity=0.05):
        for i in range(NoOfFrames):  # This shows how many
            ...
            CrystalVis(reader, FilePathForScreenshot +
                       File[i], ScalarName, opacity)
...
def Test_crystal_script(filepath):
    assert crystal_script(filepath) ==
